...
describe("plan-diff", () => {
  ...
  test("planDiffToSideBySide pairs deletions on left, additions on right", () => {
    ...
    const hasDel = s.some(
      (r) =>
        r.left?.role === "del" &&
        r.right === null &&
        (r.left?.line === "B" || r.left?.line === "C"),
    );
    const hasAdd = s.some(
      (r) =>
        r.right?.role === "add" &&
        r.left === null &&
        (r.right?.line === "B" || r.right?.line === "C"),
    );
  ...
  test("planDiffToSideBySide duplicates context on both columns", () => {
    ...
    expect(s).toEqual([
      {
        left: { line: "one", role: "ctx" },
        right: { line: "one", role: "ctx" },
      },
      {
        left: { line: "two", role: "ctx" },
        right: { line: "two", role: "ctx" },
      },
    ]);
  });
});
