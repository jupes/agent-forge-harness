...
if (!name) { console.error(JSON.stringify({ok:false,error:"Usage: scaffold.ts <name> [desc]"})); process.exit(1); }
const dir = join(process.cwd(), ".claude", "skills", name);
if (existsSync(dir)) { console.error(JSON.stringify({ok:false,error:`Already exists: ${dir}`})); process.exit(1); }
