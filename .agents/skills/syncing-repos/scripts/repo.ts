...
function output(result: CommandResult): void {
  if (humanMode) {
    ...
    for (const r of result.repos) {
      const icon = r.ok ? "✓" : "✗";
      console.log(
        `  ${icon} ${r.name}: ${r.action}${r.message ? ` — ${r.message}` : ""}`,
      );
    }
...
if (command === "init") {
  ...
  for (const repo of targets) {
    ...
    const [ahead, behind] = aheadBehind.ok
      ? aheadBehind.output.split("\t").map(Number)
      : [0, 0];
    results.push({
      name: repo.name,
      ok: true,
      ...
      path: repoPath,
      branch,
      message: `ahead:${ahead} behind:${behind} dirty:${dirty}`,
    });
  }
  ...
  for (const repo of targets) {
    const repoPath = join(REPOS_DIR, repo.name);
    if (!existsSync(repoPath)) {
      errors.push({ repo: repo.name, message: "not cloned" });
      continue;
    }
  ...
  for (const repo of targets) {
    ...
    if (dryRun) {
      results.push({
        name: repo.name,
        ok: true,
        action: `would delete: ${branches.join(", ") || "none"}`,
        path: repoPath,
      });
    } else {
      ...
      results.push({
        name: repo.name,
        ok: true,
        action: `deleted ${branches.length} branches`,
        path: repoPath,
      });
    }
  }
